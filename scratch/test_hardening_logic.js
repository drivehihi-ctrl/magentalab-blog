...
// Mock server-only in Module loader
...
// Simple TS transform via esbuild (included in next.js)
const fs = require('fs');
...
function loadTS(relPath) {
  ...
  const m = { exports: {} };
  const evalFunc = new Function('module', 'exports', 'require', '__dirname', '__filename', result.code);
  evalFunc(m, m.exports, require, path.dirname(filePath), filePath);
  return m.exports;
}

const { saveRevision } = loadTS('../lib/ai-revisions.ts');
const { applyOneRevision } = loadTS('../lib/apply-revision.ts');
