...
const { applyOneRevision } = require('../lib/apply-revision');
const { getRevision, saveRevision } = require('../lib/ai-revisions');

async function runDryRunHardeningTest() {
  console.log("=============================================================");
  console.log("[Phase 5.4 Hardening Dry-Run Test]");
  console.log("=============================================================\n");

  // Test 1: Medical content without medical_reviewed flag
  ...
  if (!res1.success && res1.error_code === 'MEDICAL_REVIEW_REQUIRED') {
    console.log("✅ TEST 1 PASSED: MEDICAL_REVIEW_REQUIRED triggered correctly\n");
  } else {
  ...
  if (res2.success && res2.backup_id === 'dry-run') {
    console.log("✅ TEST 2 PASSED: Medical content approved with flag\n");
  } else {
