...
<strong>イメージプロンプト:</strong> Hyper-realistic 3D render style, photorealistic appearance. A brown dachshund wearing round-rimmed glasses and holding a small gold magnifying glass looks as if he is wearing a white lab coat. He is a researcher, and his name is Ansim. He is gently observing a diabetic dog beside its meal bowl while a Korean pet parent records appetite, water intake, and daily routine in a notebook in a cozy Korean apartment living room. Realistic fur, natural daylight, warm trustworthy veterinary research atmosphere, no dosage numbers, no injection close-up, educational composition.</p>
...
if (fs.existsSync(csvPath)) {
  ...
  rows = rows.map((r, idx) => {
    ...
    const match = batchFullData.find(m => m.content_id === id);

    if (match) {
      updatedCount++;
      r[3] = match.title; // title
      r[5] = modifiedDateStr; // modified_date
      r[9] = cleanHtml(match.excerpt); // excerpt_clean
      r[10] = cleanHtml(match.content); // content_clean
    }
    return r;
  });
  ...
  console.log(`✅ Successfully updated ${updatedCount} rows in magentalab_all_posts_454.csv with FULL CONTENT!`);
}
