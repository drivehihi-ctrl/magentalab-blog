...
// -----------------------------------------------------------------------------
// REAL USER IMAGES HTML BLOCKS RESTORATION
// -----------------------------------------------------------------------------
...
<p><strong>「生活のリズムは安定させる。ガイドラインをその場で自分で作らない。」</strong></p>
...
async function run() {
  ...
  for (const item of postsData) {
    ...
    // 1. Update WP REST API with real user images
    ...
    console.log(`✅ WP Post ID ${updatedPost.id} successfully updated with REAL USER IMAGES!`);
  ...
  // Revalidate CDN cache immediately so user can see real images on web!
  console.log('\nRevalidating CDN Cache...');
  const revalRes = await fetch('https://www.magentalabblog.com/api/revalidate?secret=magentalab-1234');
  if (revalRes.ok) {
    console.log('✅ CDN Cache successfully purged & revalidated!');
  }

  console.log('🎉 REAL USER IMAGES RESTORATION & UPDATE COMPLETE FOR BATCH 001 - 003!');
}
