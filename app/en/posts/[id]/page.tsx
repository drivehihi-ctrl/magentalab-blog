...
export async function generateStaticParams() {
  try {
    const { posts } = await getPosts(1, 500, undefined, undefined, "en");
...
export default async function EnglishPostDetailPage({ params }: PageProps) {
  ...
  try {
    if (isNumeric) {
      ...
      // If it doesn't end with -en, check if the -en version exists
      if (!id.endsWith("-en")) {
        try {
          const enPost = await getPostBySlug(id + "-en");
          if (enPost) {
            shouldRedirect = true;
            targetSlug = enPost.slug;
            post = enPost;
          } else {
    ...
    <article className="pb-24">
      ...
      <div className="container mx-auto px-4 -mt-16 sm:-mt-20 lg:-mt-24 mb-16 relative z-20">
        <div className="max-w-5xl mx-auto rounded-3xl overflow-hidden shadow-2xl shadow-magenta/10 border-4 border-white">
          <Image
            src={imageUrl}
            alt={sanitizeForSeo(post.title.rendered) || "Magentalab Pet Research Featured Image"}
            width={1200}
            height={675}
      ...
      <section className="container mx-auto px-4 max-w-4xl -mt-12 relative z-20">
        <div className="bg-white rounded-[2rem] p-8 md:p-16 shadow-2xl shadow-gray-200/50 border border-gray-100">
          ...
          <div 
            ...
            dangerouslySetInnerHTML={{ __html: fixWpLinks(cleanContentReferences(contentHtml), sanitizeForSeo(post.title.rendered), 'en') }}
          />
            ...
            <div className="mt-12 pt-8 border-t border-gray-100">
              <div className="flex flex-wrap gap-2 text-sm">
                {tags.map((tag: any) => (
                  <Link 
                    key={tag.id} 
                    href={`/en/blog?search=${encodeURIComponent(tag.name)}`}
                    rel="nofollow"
